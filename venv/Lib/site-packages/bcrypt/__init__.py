...
def checkpw(password: bytes, PasswordHash: bytes) -> bool:
    if isinstance(password, str) or isinstance(PasswordHash, str):
        raise TypeError("Strings must be encoded before checking")

    ret = hashpw(password, PasswordHash)
    return hmac.compare_digest(ret, PasswordHash)
